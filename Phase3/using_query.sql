...
-- -> 이름이 "Spicy Paneer Wrap"인 menu_item의 이름과 menu_item의 수량, menu_item에 필요한 재료의 총 수량을 출력
...
WHERE mi.name = "Spicy Paneer Wrap"; -- 수정부분


--Q9 - type9
-- 각 menu_item에 대해 그것을 관리하는 manager의 이름과 그 menu_item이 몇 번 주문되었는지의 총 수량을 출력
-- -> 이름이 "Spicy Paneer Wrap"인 menu_item에 대해 그것을 관리하는 manager의 이름과 그 menu_item이 몇 번 주문되었는지의 총 수량을 출력
SELECT m.name AS manager_name, mi.name AS menu_item_name, SUM(po.amount) AS total_ordered
FROM menu_item mi
...
-- -> 총 가격이 100000 이상인 payment에서 customer의 이름, 주문한 menu_item의 이름 그리고 해당 menu_item을 관리하는 manager의 이름을 출력
SELECT c.name AS customer_name, mi.name AS menu_item_name, m.name AS manager_name
...
AND p.Total_price >= 100000; -- 수정부분


--Q20 - type8
-- customer가 주문한 menu_item의 이름과 해당 menu_item을 만드는 chef의 이름을 출력. 결과는 chef의 이름 순으로 정렬
-- -> customer_id가 "CU000001" 인 customer가 주문한 menu_item의 이름과 해당 menu_item을 만드는 chef의 이름을 출력. 결과는 chef의 이름 순으로 정렬
SELECT c.name AS customer_name, mi.name AS menu_item_name, ch.name AS chef_name
FROM customer c, payment p, part_of po, menu_item mi, chef ch
WHERE c.customer_id = p.customer_id
AND p.order_id = po.order_id
AND po.item_id = mi.item_id
AND p.chef_id = ch.chef_id
AND c.customer_id = "CU000001" -- 수정부분
ORDER BY ch.name;
