...
AFTER INSERT OR UPDATE ON need
FOR EACH ROW
DECLARE
    v_item_id CHAR(8);
...
BEGIN
    ...
    SELECT SUM(i.quantity) INTO v_total_quantity
    FROM ingredient i
    ...
    ELSE
        -- quantity가 0보다 크면 해당하는 재료의 합계로 item_quantity를 업데이트
        UPDATE menu_item
        SET item_quantity = v_total_quantity
        WHERE item_id = v_item_id;
    END IF;
END;
/
